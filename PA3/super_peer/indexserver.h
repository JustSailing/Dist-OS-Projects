...
// struct to help with vector holding information on connected super nodes
struct SuperNode
{
    std::string ip;
    std::string port;
    // Message mess;
    // bool super_peer;     // if super peer, had problems bc you cannot connect to peer as a client
    int c_sock; // needed since you can not connect to peer client just peer server
};
...
public:
    ...
    */
    IndexServer(const std::string &log_file, const std::string &csv_file,
                const char *ip_addr, const char *port_number, std::vector<SuperNode> vec);
...
private:
    ...
    // vector holding unique pointers to super nodes connected [changed]
    std::vector<SuperNode> m_super_nodes;
    std::mutex m_lock_pairs;
    ...
    */
    ...
    /*
        ...
        relative to message
    */
    void Receive(int c_sock, std::string_view ip, std::string_view port,
                 std::chrono::system_clock::time_point &time) noexcept;

    /*
        Get list of peers of have the file
    */
    [[nodiscard]] bool GetPeerNodeList(const Message &msg, int peer_sock, std::string_view ip, std::string_view port) noexcept;
    ...
    */
    ...
    void Query(const Message &message, int peer_sock, std::string_view ip, std::string_view port,
               const std::chrono::system_clock::time_point &start) noexcept;

    void QueryHit(const Message &message, int peer_sock, std::string_view ip,
               std::string_view port, const std::chrono::system_clock::time_point &start) noexcept;
