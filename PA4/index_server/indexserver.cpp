...
void IndexServer::Start() noexcept
{
    ...
    std::array<std::thread, 20> all_threads;
...
bool IndexServer::AddNewFile(const Message &msg, int c_sock, std::string_view ip) noexcept
{
    ...
    bool check = false;
    {
        ...
        for (auto &peer : m_peer_nodes)
        {
            ...
            {
                peer->files_map.push_back(std::make_pair(_file, std::to_string(msg.filesize)));
                check = true;
                break;
            }
        }
    ...
    auto check = -1;
    {
        ...
        for (auto &peer : m_peer_nodes)
        {
            if (peer->id == msg.peer_id)
            {
                ...
                {
                    if (peer->files_map[i].first == _file)
                    {
                        check = i;
                        break;
                    }
...
int IndexServer::RegisterNode(Message &msg, int c_sock, std::string_view ip) noexcept
{
    auto peer_id = 0;
    std::string event;
    ...
    for (auto i = 0; i < vc.size(); i += 2)
    {
        peer->files_map.push_back(std::make_pair(vc[i], vc[i + 1]));
    }
...
void IndexServer::Register(Message &message, int peer_sock, std::string_view ip,
                           const std::chrono::system_clock::time_point &start) noexcept
{
    std::string event;
