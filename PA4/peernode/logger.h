 ...
 *  Allows peer node to write events to files on its storage to help keep track of 
...
public:
    ...
    // Logs events made by peer node with just time stamp to m_log_file
    // Example: socket creation, bind, listen, accept
    void Log(std::string_view event) noexcept;

    // Logs events made by peer node with time stamp, start time, end time,
    // and the difference
...
private:
    // Output file stream that the peer node uses to write to file
    std::ofstream m_log_file_stream;

    // File to log all peer node events
    std::string m_log_file;

    // Output file stream that the peer node uses to log time differences in csv style
