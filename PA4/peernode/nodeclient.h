...
private:
    ...
    [[nodiscard]] bool AddFile(const std::string &filename, const uint64_t &filesize, const std::chrono::system_clock::time_point &start) noexcept;
    void Download(const std::string& ip, const std::string& port,
                  const std::string& filename, const uint64_t filesize, uint64_t &chunks, std::mutex &mut_cks, std::mutex &f_file_lock,
