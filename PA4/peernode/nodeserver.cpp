...
bool NodeServer::Register() noexcept
{
    ...
    if (mess->mess_id == UPDATE)
    {
        ...
        if (all_files.empty())
        {
            ...
            auto mess2 = reinterpret_cast<Message *>(buffer);
            m_logger->Log(std::to_string(mess2->mess_id));
            if (mess2->mess_id == COMPLETED)
            {
        ...
        std::vector<std::string> all;
        {
            ...
            {
                if ((counter + file.first.length() + file.second.length()) < CHUCK_SIZE)
                {
                    f += file.first + ",";
                    f += file.second + ",";
                    counter += file.first.length() + file.second.length();
                }
                else
                {
                    all.push_back(f);
        ...
        while (count < 11)
        {
            ...
            auto mess2 = reinterpret_cast<Message *>(buffer);
            m_logger->Log(mess2->body);
            m_logger->Log(std::to_string(mess2->mess_id));
            if (mess2->mess_id == COMPLETED)
            {
                event = "[SUCCESS] Registering with index server ";
                m_peer_id = mess2->peer_id;
                peer_id = mess2->peer_id;
                FillLog(event, "REGISTER", m_super_ip, fd, start);
                return true;
            }
            count++;
...
{
    std::array<std::thread, 20> server_threads;
                         ...
                         std::chrono::system_clock::time_point &start) noexcept
{
    ...
    default:
    {
        event = "[ERROR] Peer acting as a server should only serve download request  " + std::to_string(msg->mess_id) + " [TIME] ";
        m_logger->Log(event);
        return;
    }
    }
                          ...
                          const std::chrono::system_clock::time_point &start) noexcept
{
    ...
    f.seekg(0);
    // f.seekg(msg.offset * 1024);
    // char buffer[CHUCK_SIZE];
    Message mess_out{};
    if (msg.offset == 0 && filesize < CHUCK_SIZE)
    {
        // f.seekg(0);
        ...
        mess_out.offset = msg.offset;
        mess_out.peer_id = m_peer_id;
    ...
    else if (((int)filesize - (msg.offset * CHUCK_SIZE)) < 0)
    {
        f.seekg(msg.offset * CHUCK_SIZE);
        char buff[CHUCK_SIZE];
        bzero(buff, sizeof(buff));
        mess_out.mess_id = DOWNLOAD;
        mess_out.size = (msg.offset * CHUCK_SIZE) - (int)filesize;
        mess_out.filesize = filesize;
        mess_out.offset = msg.offset;
        mess_out.mess_in_seq = 1;
        mess_out.total_mess = 1;
        mess_out.peer_id = m_peer_id;
        f.read(buff, (int)filesize - (msg.offset * CHUCK_SIZE));
        // m_log->Log(buffer);
        std::strncpy(mess_out.body, buff, sizeof(buff));
        if (send(peer_sock, &mess_out, sizeof(mess_out), 0) <= 0)
        {
    ...
    else
    {
        ...
        bzero(buff, sizeof(buff));
        mess_out.mess_id = DOWNLOAD;
        mess_out.size = CHUCK_SIZE;
        mess_out.filesize = filesize;
        mess_out.offset = msg.offset;
        mess_out.mess_in_seq = 1;
        mess_out.total_mess = 1;
        mess_out.peer_id = m_peer_id;
        f.read(buff, CHUCK_SIZE);
        // m_log->Log(buffer);
        std::strncpy(mess_out.body, buff, sizeof(buff));
        if (send(peer_sock, &mess_out, sizeof(mess_out), 0) <= 0)
        {
